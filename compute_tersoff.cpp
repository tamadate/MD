...
/////////////////////////////////////////////////////////////////////
void
Tersoff::compute_tersoff(Variables *vars) {
...
double
Tersoff::compute_tersoff_virial(Variables *vars) {
/*	Ion *ions = vars->ions.data();
	const int is = vars->ions.size();
	double virial=0;
	for(auto &a : pairs){
...
void
Tersoff::attractive(double prefactor, double rij, double rik, double *delrij, double *delrik, double *fi, double *fj, double *fk){
	double rij_hat[3],rik_hat[3];
	for (int i=0; i<3; i++) {
...
void 
Tersoff::zeta_d(double prefactor, double *rij_hat, double rij,double *rik_hat, double rik,double *dri, double *drj, double *drk){
...
double 
Tersoff::force_zeta(double r, double zeta_ij, double &prefactor){
...
double
Tersoff::zeta(double rij, double rik, double x1x2, double y1y2, double z1z2) {
...
void
Tersoff::check_pairlist(Variables *vars){
	loop_t++;
	if(loop_t>loop_update_t) make_pair(vars);
}

void
Tersoff::make_pair(Variables *vars){
	pairs.clear();
	Ion *ions = vars->ions.data();
