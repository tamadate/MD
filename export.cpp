...
MD::export_dump(void) {
	...
	for (auto &a : vars->ions) fprintf(f,"%d %d %f %f %f\n",a.id,a.type,a.qx-X,a.qy-Y,a.qz-Z);
	if(flags->dump_gas) {
        for (auto &a : gas_out) fprintf(f, "%d %d %f %f %f\n", vars->gases[a].id, vars->gases[a].type-1, vars->gases[a].qx-X, vars->gases[a].qy-Y, vars->gases[a].qz-Z);
        for (auto &a : gas_in) fprintf(f, "%d %d %f %f %f\n", vars->gases[a].id, vars->gases[a].type, vars->gases[a].qx-X, vars->gases[a].qy-Y, vars->gases[a].qz-Z);
    }
	fclose(f);
}
