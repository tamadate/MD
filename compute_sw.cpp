...
double
SW::compute_sw_virial(Variables *vars) {
	Ion *ions = vars->ions.data();
	const int is = vars->ions.size();
/*	double virial=0;
...
void 
SW::threebody(double rsq1, double rsq2,double *rij, double *rik,double *fj, double *fk)
{
...
void
SW::check_pairlist(Variables *vars){
	loop_t++;
	if(loop_t>loop_update_t) make_pair(vars);
}

void
SW::make_pair(Variables *vars){
	pairs.clear();
	Ion *ions = vars->ions.data();
